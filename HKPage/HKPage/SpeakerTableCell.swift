...
class SpeakerTableCell: UITableViewCell {
    ...
    */
    @IBAction func micButtonDidPress(sender: AnyObject) {
        ...
        println("micButton.selected: \(micButton.selected)")
        if micButton.selected {
            if !deviceInfo.active {
                println("addDeviceToSession: \(deviceInfo.deviceId)")
                HKWControlHandler.sharedInstance().addDeviceToSession(deviceInfo.deviceId)
            }
        }
        else {
            if deviceInfo.active {
                println("removeDeviceFromSession: \(deviceInfo.deviceId)")
