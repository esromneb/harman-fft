...
//        HKWControlHandler.debugPrintOn(true)
        ...
        if g_HWControlHandler == nil {
            ...
            // Create Firecast Control Handler
            g_HWControlHandler = HKWControlHandler()
    ...
    override func viewDidAppear(animated: Bool) {
        alert = UIAlertController(title: "Finding HKWirelessHD Network", message: "If this dialog does not disappear, please check if any other HK WirelessHD App is running on the phone and kill it. Or, your phone is not in a Wifi network.", preferredStyle: .Alert)
    ...
    func startHKWirelessHD() {
        ...
        if g_HWControlHandler.isInitialized() {
            return
        }
        
        if g_fcInitialized == false {
            // firecaset has not been started before. Start now
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), {
                if g_HWControlHandler.initializeHKWirelessController(kLicenseKeyGlobal) != 0 {
