...
class RadarSearchingViewController: UIViewController {
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), {
            while (!self.stopQuerying) {
                sleep(2)
                var deviceCount = g_HWControlHandler.getDeviceCount()
                for var i = 0; i < deviceCount; i++ {
                    let deviceInfo = g_HWControlHandler.getDeviceInfoByIndex(i)
                    g_HWControlHandler.refreshDeviceWiFiSignal(deviceInfo.deviceId)
                }
    ...
    */
    func startSearch() {
        ...
        speakers = [DeviceInfo]()
        nData = g_HWControlHandler.getDeviceCount()

        for var i = 0; i < nData; i++ {
            speakers.append(g_HWControlHandler.getDeviceInfoByIndex(i))
        }
